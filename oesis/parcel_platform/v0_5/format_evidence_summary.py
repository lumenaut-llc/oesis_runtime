...
def _node_source_mix(payload: dict) -> dict:
    """Summarize evidence sources by node and source class."""
    explanation = payload.get("explanation_payload", {})
    contributions = explanation.get("evidence_contributions", [])
    local_nodes = sorted({c["source_name"] for c in contributions if c.get("source_class") == "local" and "source_name" in c})
    source_classes = sorted({c["source_class"] for c in contributions if "source_class" in c})
    return {
        "evidence_source_nodes": local_nodes,
        "evidence_source_classes": source_classes,
        "observation_count": payload.get("provenance_summary", {}).get("observation_count", 0),
    }
...
def build_evidence_summary(payload: dict) -> dict:
    ...
    return {
        ...
        "node_source_mix": _node_source_mix(payload),
