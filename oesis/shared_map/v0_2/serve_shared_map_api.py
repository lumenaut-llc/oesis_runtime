...
from .aggregate_shared_map import (
    SharedMapError,
    aggregate_shared_map,
    eligibility_from_consents,
...
class SharedMapRequestHandler(BaseHTTPRequestHandler):
    ...
    def _configured_consent_store(self):
    ...
    def do_POST(self):
        if self.path in {"/v1/shared-map/tiles", "/v1/admin/shared-map/inspect"}:
            try:
                ...
                consent_store = self._configured_consent_store()
                if self.path == "/v1/shared-map/tiles":
                    ...
                    result = build_shared_map_inspection(payload, sharing_store=sharing_store, consent_store=consent_store)
