# Runtime Examples (`v0.1`)

This directory holds baseline runtime fixtures for the frozen `v0.1` lane.

## Baseline fixtures (gating for `v0.1` checks)
...
## Transitional compatibility fixtures (non-gating)
...
and `v2.5` controls-compatibility inventory), even when compatibility copies
exist in `v0.1`.
