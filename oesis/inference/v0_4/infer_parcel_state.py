...
def compose_multi_node_evidence(observations: list[dict], parcel_context: dict | None = None) -> dict:
    """Compose evidence from multiple normalized observations.

    Weights evidence by node calibration state, freshness, and location_mode coverage.
    Returns a composed evidence dict with per-node contributions.
    """
    if not observations:
        return {"observation_count": 0, "contributions": [], "source_diversity": {}}

    contributions = []
    location_modes_seen = set()
    observation_types_seen = set()

    for obs in observations:
        node_id = obs.get("node_id", "unknown")
        context = classify_local_context(obs, parcel_context=parcel_context)
        location_mode = context.get("location_mode", "unknown")
        location_modes_seen.add(location_mode)

        obs_type = obs.get("observation_type", "environmental.snapshot")
        observation_types_seen.add(obs_type)

        registry_meta = obs.get("provenance", {}).get("registry_metadata", {})
        cal_state = registry_meta.get("calibration_state", "unknown")

        cal_weights = {"provisional": 0.95, "verified": 1.1, "recently_calibrated": 1.05, "unknown": 0.8}
        cal_weight = cal_weights.get(cal_state, 0.8)

        contributions.append({
            "node_id": node_id,
            "observation_type": obs_type,
            "location_mode": location_mode,
            "install_role": context.get("install_role", "unknown"),
    ...
    return {
        ...
        "source_diversity": {
            "location_modes": sorted(location_modes_seen),
            "observation_types": sorted(observation_types_seen),
            "indoor_count": sum(1 for c in contributions if c["location_mode"] == "indoor"),
            "outdoor_count": sum(1 for c in contributions if c["location_mode"] == "outdoor"),
            "sheltered_count": sum(1 for c in contributions if c["location_mode"] == "sheltered"),
        },
    }
