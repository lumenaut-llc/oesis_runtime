...
def eligibility_from_consents(
    store: dict,
    *,
    data_class: str = "outdoor_pm25",
    recipient_type: str = "neighborhood_pool",
) -> dict:
    eligibility: dict[str, bool] = {}
    for record in store.get("consents", []):
        parcel_ref = record.get("parcel_ref")
        if not parcel_ref:
            continue
        is_active = record.get("revoked_at") is None
        is_shared = record.get("custody_tier") == "shared"
        recipient_matches = record.get("recipient_type") == recipient_type
        class_allowed = data_class in record.get("data_classes", [])
        if is_active and is_shared and recipient_matches and class_allowed:
            eligibility[parcel_ref] = True
    return eligibility
...
def aggregate_shared_map(
    ...
    *,
    ...
    eligibility = (
        eligibility_from_consents(consent_store)
        if consent_store is not None
        else eligibility_from_store(sharing_store)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--consent-store",
        default=None,
        help="Optional path to a JSON consent store file used for operational consent gating.",
    )
...
def main() -> int:
    ...
    try:
        ...
        consent_store = load_consent_store(Path(args.consent_store).resolve()) if args.consent_store else None
