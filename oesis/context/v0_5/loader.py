"""Context loader for v0.5: governance enforcement (consent, retention, export, revocation)."""
...
def load_default_bundle(*, parcel_id: str = DEFAULT_PARCEL_ID, include_support_objects: bool = False) -> dict:
    ...
    flood_packet = _try_load_example("node-observation-flood.example.json")
    if flood_packet is not None:
        flood_packet["node_id"] = "flood-node-01"
        bundle["flood_packet"] = flood_packet
    node_registry = _try_load_example("node-registry.example.json")
    if node_registry is not None:
        bundle["node_registry"] = node_registry
    sharing_settings = _try_load_example("sharing-settings.example.json")
    if sharing_settings is not None:
        bundle["sharing_settings"] = sharing_settings
    consent_store = _try_load_example("consent-store.example.json")
    if consent_store is not None:
        bundle["consent_store"] = consent_store
