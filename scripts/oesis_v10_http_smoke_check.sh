...
echo "[oesis-v10-http-check] starting ingest api (port ${OESIS_HTTP_INGEST_PORT})"
python3 -m oesis.ingest.serve_ingest_api --host 127.0.0.1 --port "${OESIS_HTTP_INGEST_PORT}" >/tmp/oesis-v10-ingest.log 2>&1 &
INGEST_PID=$!

echo "[oesis-v10-http-check] starting inference api (port ${OESIS_HTTP_INFERENCE_PORT})"
python3 -m oesis.inference.serve_inference_api --host 127.0.0.1 --port "${OESIS_HTTP_INFERENCE_PORT}" >/tmp/oesis-v10-inference.log 2>&1 &
INFERENCE_PID=$!

echo "[oesis-v10-http-check] starting parcel-platform api (port ${OESIS_HTTP_PARCEL_PORT})"
python3 -m oesis.parcel_platform.serve_parcel_api --host 127.0.0.1 --port "${OESIS_HTTP_PARCEL_PORT}" >/tmp/oesis-v10-parcel.log 2>&1 &
PARCEL_PID=$!

echo "[oesis-v10-http-check] waiting for health endpoints"
for i in $(seq 1 "${OESIS_HTTP_HEALTH_RETRIES}"); do
  if curl -sf "http://127.0.0.1:${OESIS_HTTP_INGEST_PORT}/v1/ingest/health" >/tmp/oesis-v10-ingest-health.json \
    && curl -sf "http://127.0.0.1:${OESIS_HTTP_INFERENCE_PORT}/v1/inference/health" >/tmp/oesis-v10-inference-health.json \
    && curl -sf "http://127.0.0.1:${OESIS_HTTP_PARCEL_PORT}/v1/parcel-platform/health" >/tmp/oesis-v10-parcel-health.json; then
    break
  fi
  if [[ "$i" -eq "${OESIS_HTTP_HEALTH_RETRIES}" ]]; then
    echo "[oesis-v10-http-check] ERROR: services did not become healthy in time (see /tmp/oesis-v10-*.log)" >&2
    exit 1
  fi
...
echo "[oesis-v10-http-check] posting node packet to ingest api"
curl -s -X POST "http://127.0.0.1:${OESIS_HTTP_INGEST_PORT}/v1/ingest/node-packets" \
  -H 'Content-Type: application/json' \
  -H 'X-OESIS-Parcel-Id: parcel_demo_http_v10' \
  --data-binary @"$NODE_PACKET_EXAMPLE" \
  >/tmp/oesis-v10-ingest-response.json

echo "[oesis-v10-http-check] building v1.0 inference request (normalized + parcel + public context)"
python3 - <<'PY'
import json
from pathlib import Path
...
Path("/tmp/oesis-v10-inference-request.json").write_text(
    json.dumps(body),
    encoding="utf-8",
)
PY
...
curl -s -X POST "http://127.0.0.1:${OESIS_HTTP_INFERENCE_PORT}/v1/inference/parcel-state" \
  ...
  --data-binary @/tmp/oesis-v10-inference-request.json \
  >/tmp/oesis-v10-inference-response.json

echo "[oesis-v10-http-check] posting parcel state to parcel-platform api"
python3 - <<'PY'
import json
from pathlib import Path

payload = json.loads(Path("/tmp/oesis-v10-inference-response.json").read_text(encoding="utf-8"))
Path("/tmp/oesis-v10-parcel-state-from-http.json").write_text(
...
curl -s -X POST "http://127.0.0.1:${OESIS_HTTP_PARCEL_PORT}/v1/parcels/state/view" \
  -H 'Content-Type: application/json' \
  --data-binary @/tmp/oesis-v10-parcel-state-from-http.json \
  >/tmp/oesis-v10-parcel-response.json

echo "[oesis-v10-http-check] checking response shapes (v1.0 lane)"
python3 - <<'PY'
import json
from pathlib import Path
...
ingest_health = load("/tmp/oesis-v10-ingest-health.json")
inference_health = load("/tmp/oesis-v10-inference-health.json")
parcel_health = load("/tmp/oesis-v10-parcel-health.json")
ingest_payload = load("/tmp/oesis-v10-ingest-response.json")
inference_payload = load("/tmp/oesis-v10-inference-response.json")
parcel_payload = load("/tmp/oesis-v10-parcel-response.json")
...
print("PASS oesis-v10-http-check")
PY
